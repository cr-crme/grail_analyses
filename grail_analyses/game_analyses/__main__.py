...
def Excel(ID, file, directory):
    ...
    # Creation des feuilles de jeu
    games = [obj for name, obj in inspect.getmembers(jeux) if inspect.isclass(obj)]
    for game in games:
        classJeux = game()
        
        motif_recherche = f'{directory}/{ID}/Record Data/Fichiers txt/{classJeux.nom_txt}'
        fichiers_correspondants = glob.glob(motif_recherche, recursive=True)
        
        if classJeux.nom in wb.sheetnames:
            wb.remove(wb[classJeux.nom])
            
        if len(fichiers_correspondants) > 0:
            ws = wb.create_sheet(classJeux.nom)
            ...
            for fichier in fichiers_correspondants:
                ...
                resultats = classJeux.calculs(donnees=data, AAAA=AAAA, MM=MM, DD=DD)
